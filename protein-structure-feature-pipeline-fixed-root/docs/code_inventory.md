...
| `02_compute_local_tracks.py` | Compute structure-derived local tracks: DSSP, RSA/buried residues, hydropathy, approximate clashes, bond outliers, and cis-peptide flags | local features |
| `03_compute_disorder_qscore.py` | Add optional IUPred2A disorder and ChimeraX/QScore features | disorder / validation |
| `04_reparse_qscore_raw.py` | Optional utility for reparsing saved QScore raw text and writing it back to JSON | validation utility |
...
| `tools/emdb/emdbMatch_relaxed.py` | Generate or relax chain correspondence from CryoNet/emdbMatch-style inputs | optional helper |
| `tools/emdb/run_emdbmatch_nosource_modify.py` | Run emdbMatch for selected problem entries and classify output usability | optional helper |
| `tools/diagnostics/*.py` | Inspect batch/web files and individual cases | diagnostics |
| `tools/hpc/split_manifest_3000.py` | Split large manifests into batch-sized chunks | HPC helper |
