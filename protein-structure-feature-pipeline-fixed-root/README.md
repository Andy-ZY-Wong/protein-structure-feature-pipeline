...
scripts/          Executable Python scripts for the standard pipeline stages
tools/            Optional diagnostic, EMDB-matching, and HPC helper utilities
...
| 02 | `02_compute_local_tracks.py` | Compute local structure-derived tracks such as DSSP, RSA, hydropathy, approximate clashes, bond outliers, and cis-peptide flags |
| 03 | `03_compute_disorder_qscore.py` | Add optional IUPred2A disorder and ChimeraX/QScore features |
| 04 | `04_reparse_qscore_raw.py` | Optional utility for reparsing saved ChimeraX QScore raw outputs |
...
Stage 02 is intentionally limited to local structure-derived tracks. Disorder and Q-score features are handled in Stage 03 so that optional external validation features are separated from the core local-track calculation. Additional diagnostic, EMDB-matching, and manifest-splitting utilities are kept under `tools/` and are not required for the standard demo workflow.
...
This repository presents a reproducible structural bioinformatics workflow for processing protein structure data at the chain and residue levels. It demonstrates how raw structure files, sequence records, and annotation resources can be converted into structured JSON/CSV outputs through a modular Python/Linux pipeline.

The workflow includes structure parsing, local feature extraction, UniProt/SIFTS-based chain annotation, problem-chain repair, quality-control reporting, and optional Slurm-based batch execution. Small synthetic example data are included so that the project can be inspected and tested without relying on production-scale infrastructure.
